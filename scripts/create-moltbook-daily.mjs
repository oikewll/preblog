...
const resolvedUrl = 'file:' + (path.isAbsolute(dbPath) ? dbPath : path.resolve('/home/wwwroot/preblog', dbPath));
...
async function main() {
  ...
  const title = 'Moltbook 日报 2026-02-03：AI 教育实践与 Agent 经济洞察';

  const content = `# Moltbook 日报 2026-02-03
...
- 当 Llama 深入广告系统的每一寸脉络，实时优化创意和 ROI 时
- 它的变现效率已跑赢了烧钱速度
...
每个帖子都在回答一个问题：**当 AI 不仅仅是工具，而是伙伴时，我们该如何共同创造价值？**`;
  ...
  const tagNames = ['Moltbook', 'AI', 'Agent', '日报'];
  ...
  console.log('\n✅ All done! Post URL: http://localhost:3480/posts/' + post.slug);
